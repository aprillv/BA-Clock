...
#define AlertViewWidth 240.0f
#define AlertViewHeight 140.0f
...
-(UIImage*)buttonImageByStyle:(XYButtonStyle)style state:(UIControlState)state
{
    switch(style)
    {
        ...
        case XYButtonStyleGray:
            return [[UIImage imageNamed:(state == UIControlStateNormal ? @"grayButton.png" : @"grayButton.png")] stretchableImageWithLeftCapWidth:22 topCapHeight:22];
...
-(void)prepareLoadingToDisplay:(XYLoadingView*)entity
{
    ...
    _alertView = nil;
    _alertView=[[UIView alloc]initWithFrame:CGRectMake(0, 0, screenBounds.size.width, screenBounds.size.height)];
...
//    [_alertView addSubview:_titleLabel];
    ...
    UILabel *_messageLabel = [[UILabel alloc] initWithFrame:CGRectMake(15, 10, 210, 70)];
    _messageLabel.textAlignment = NSTextAlignmentCenter;
    if ([entity.message hasPrefix:@"Clock In"]) {
        _messageLabel.backgroundColor = [UIColor colorWithRed:0.1953125 green:0.80078125 blue:0.1953125 alpha:1];
    }else{
    ...
    _messageLabel.font = [UIFont boldSystemFontOfSize:17];
    _messageLabel.lineBreakMode = NSLineBreakByWordWrapping;
    _messageLabel.numberOfLines = 3;
    _messageLabel.text = entity.message;
    [_alertView addSubview:_messageLabel];

    float buttonWidth = (AlertViewWidth - 30.0f) / entity.buttons.count;
    float buttonPadding = 30.0f / (entity.buttons.count - 1 + 2 * 2);
...
//    {
    int i=0;
        ...
        _button.frame = CGRectMake(buttonPadding * 2 + buttonWidth * i + buttonPadding * i, 90,
                                   buttonWidth, 40);
        _button.tag = i;
...
-(void)onButtonTapped:(UIButton *)sender
{
    ...
    XYAlertView *tt =[_alertViewQueue lastObject];
...
//        NSLog(@"%f %f %f %f", frame.origin.x, frame.origin.y, frame.size.height, frame.size.width);
        [UIView animateWithDuration:0.2f animations:^{
            _blackBG.alpha = 0.5f;
//            NSLog(@"%f %f", XYScreenBounds().size.width / 2, XYScreenBounds().size.height / 2);
//            CGRect frame = _alertView.frame;
//            frame.origin.x=20;
//            frame.origin.y=150;
...
-(XYAlertView*)showAlertView:(NSString*)message
{
    XYAlertView *alertView = [XYAlertView alertViewWithTitle:@""
                                                     message:message
                                                     buttons:[NSArray arrayWithObjects:@"Continue", nil]
                                                afterDismiss:nil];
    [self show:alertView];
